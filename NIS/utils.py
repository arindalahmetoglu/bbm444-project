...
def make_cell(coord, img, sizes=None):
    if sizes is not None:
        h, w = sizes
        device = coord.device
        coord_bot_left  = coord + torch.tensor([-1/h, -1/w], device=device).unsqueeze(0)
        coord_bot_right = coord + torch.tensor([-1/h,  1/w], device=device).unsqueeze(0)
        coord_top_left  = coord + torch.tensor([ 1/h, -1/w], device=device).unsqueeze(0)
        coord_top_right = coord + torch.tensor([ 1/h,  1/w], device=device).unsqueeze(0)
        coord_left  = coord + torch.tensor([-1/h, 0], device=device).unsqueeze(0)
        coord_right = coord + torch.tensor([ 1/h, 0], device=device).unsqueeze(0)
        coord_bot   = coord + torch.tensor([ 0, -1/w], device=device).unsqueeze(0)
        coord_top   = coord + torch.tensor([ 0,  1/w], device=device).unsqueeze(0)

    else:
        device = coord.device
        img_h, img_w = img.shape[-2], img.shape[-1]
        coord_bot_left  = coord + torch.tensor([-1/img_h, -1/img_w], device=device).unsqueeze(0)
        coord_bot_right = coord + torch.tensor([-1/img_h,  1/img_w], device=device).unsqueeze(0)
        coord_top_left  = coord + torch.tensor([ 1/img_h, -1/img_w], device=device).unsqueeze(0)
        coord_top_right = coord + torch.tensor([ 1/img_h,  1/img_w], device=device).unsqueeze(0)
        coord_left  = coord + torch.tensor([-1/img_h, 0], device=device).unsqueeze(0)
        coord_right = coord + torch.tensor([ 1/img_h, 0], device=device).unsqueeze(0)
        coord_bot   = coord + torch.tensor([ 0, -1/img_w], device=device).unsqueeze(0)
        coord_top   = coord + torch.tensor([ 0,  1/img_w], device=device).unsqueeze(0)
