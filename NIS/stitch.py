...
def main(config_, args):
    ...
    tgt_img = Image.open(args.tgt).convert('RGB')
    ...
    if args.scale != 1.0:
        ...
        ref_img = ref_img.resize((new_ref_width, new_ref_height), Image.LANCZOS)
        tgt_img = tgt_img.resize((new_tgt_width, new_tgt_height), Image.LANCZOS)
        
        print(f"Resized reference image from {ref_width}x{ref_height} to {new_ref_width}x{new_ref_height}")
        print(f"Resized target image from {tgt_width}x{tgt_height} to {new_tgt_width}x{new_tgt_height}")
