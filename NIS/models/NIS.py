...
class NeuralImageStitching(nn.Module):
    ...
    def NeuralWarping(self, img, feat, feat_coord, freq, coef, coord, cell, sizes):
        ...
        # Clone the cell tensor and prepare for scaling
        rel_cell = cell.clone()
        
        # Check if cell has the right format
        if cell.dim() >= 2 and cell.shape[-1] == 2:
            # Cell needs to be properly formatted for shape estimation
            # The make_cell function creates a cell with 9 points (4 corners, 4 sides, 1 center)
            # and all 9 points have (y, x) coordinates
            # Cell is expected to be [B, N, 2] where N is 9*num_pixels
            rel_cell_reshaped = rel_cell.reshape(b, -1, 9, 2)
            
            # Scale the cell coordinates appropriately
            rel_cell_reshaped[..., :, 0] *= feat.shape[-2]  # scale y-coordinates
            rel_cell_reshaped[..., :, 1] *= feat.shape[-1]  # scale x-coordinates
            
            # Reshape to the format needed for phase function
            # The phase function expects shape [B, 10, H, W]
            rel_cell = rel_cell_reshaped.reshape(b, h, w, 18)
            # Convert from [B, H, W, 18] to [B, 18, H, W]
            rel_cell = rel_cell.permute(0, 3, 1, 2)
            # The phase function takes only the first 10 values
            rel_cell = rel_cell[:, :10, :, :]
        else:
            # Original implementation from the code
            # Make sure rel_cell has the right shape
            if rel_cell.dim() < 3:
                # Reshape to [B, N, D]
                rel_cell = rel_cell.reshape(b, -1, rel_cell.shape[-1])
            
            if rel_cell.shape[-1] >= 10:
                # Scale the appropriate indices
                rel_cell[..., [0, 2, 4, 6, 8]] *= feat.shape[-2]
                rel_cell[..., [1, 3, 5, 7, 9]] *= feat.shape[-1]
                # Reshape to [B, H, W, 10]
                rel_cell = rel_cell.reshape(b, *sizes, 10).permute(0, 3, 1, 2)

        # Fall back to original implementation which is more stable
